...
async def deal_message(msg: Message, state: FSMContext):
    ...
    if msg.text:
        ...
        await bot.send_voice(author, voice=msg.voice.file_id, caption=f"Запрос на просмотр от {msg.from_user.id}")
    now = datetime.utcnow()
...
async def commission_message(msg: Message, state: FSMContext):
    ...
    if msg.text:
        ...
        await bot.send_voice(author, voice=msg.voice.file_id, caption=f"Обсуждение комиссионных от {msg.from_user.id}")
